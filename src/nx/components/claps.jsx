...
export default class ClapHand extends React.Component {
    constructor(props) {
      super(props)
      this.state = {
        count: 0,
  ...
  // Get document instance from Firebase      
    async getInitialData(myCollection) {
      const docInstance = await doc(myCollection, this.props.page_claps)
      const docData     = await getDoc(docInstance)
      console.log('DOC instance', docInstance, this.props.page_claps)
      console.log('DOC data', docData.data())
      this.setState({myDoc:docInstance})
      this.setState({originData:docData.data()})
      this.setState({countTotal:this.state.originData.claps_count})
      console.log('STATE data', this.state.myDoc, this.state.originData, this.state.countTotal)
    }

    // Set new count in document
    // https://firebase.google.com/docs/firestore/manage-data/add-data
    async setDocData () {
      if (this.state.myDoc !== null) {
        await setDoc(this.state.myDoc, this.state.originData)
      }
      console.log('NEW clap counts', this.state.myDoc, this.state.originData, this.state.countTotal)
    }
    
    componentDidMount() {   
      this.getInitialData(myCollection)
    ...
    _handleClick () {
      ...
      // Set Firebase count
      const newCountTotal = this.state.countTotal + 1
